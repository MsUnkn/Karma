...
-- SoulShard bar  -- sorry demo :( one day I'll get it working for you
...
local Colors = {  
	...
	local totalShards = UnitPowerMax("player", SPELL_POWER_SOUL_SHARDS)
for i= 1, totalShards do
		local Shards = CreateFrame("StatusBar", nil, ShardsFrame)
		Shards:SetSize((self.Health:GetWidth() / 3)-6, 6)
		...
		if (i == 1) then
			Shards:SetPoint("BOTTOMLEFT", self.Health, "TOPLEFT", 3, 6)
		else
			Shards:SetPoint("TOPLEFT", ShardsFrame[i-1], "TOPRIGHT", 6, 0)
		end

		ShardsFrame[i] = Shards
	end
	self.Shards = ShardsFrame
	self.Shards.Override = ShardsOverride
end 
